...
A partir da **Semana 34 (Aula 03)**, Qualidade 2026.2 tem um case fio-condutor **das aulas**: **Foot Fanatics** — API de conteúdo esportivo (espinha dorsal do `remo-api`) **com assinatura**: conta, controle de acesso e gestão de sessão. É o que se codifica ao vivo em aula.

> ⚠️ **Foot Fanatics ≠ projeto avaliado.** O trabalho semestral das equipes é **"Organização de Recursos"** (alocação de salas, professores e materiais sem conflito), em `pages/qualidade2/especificacao-projeto.html`. Não troque o case dessa página pelo Foot Fanatics: a aula demonstra a técnica no Foot Fanatics, as equipes aplicam no projeto delas. No `config/semestres.json`, `case` é o case de aula e `projeto_avaliado` é o trabalho.
