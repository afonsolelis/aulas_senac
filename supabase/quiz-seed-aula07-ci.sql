...
with novas as (
  ...
  ('ci-q2-a07', 1,
   ...
   '["O primeiro job ainda não terminou, e o resumo precisa esperar o processo concluir",
     ...
     "O runner apaga os arquivos temporários assim que o passo que os gerou termina"]'::jsonb, 90, 'Anatomia do workflow', 'seção 2'),
  ...
  ('ci-q2-a07', 2,
   ...
   '["Porque test executa apenas a classe principal e ignora as classes de teste aninhadas",
     ...
     "Porque verify avança o ciclo e acomoda integração e verificações sem trocar o contrato"]'::jsonb, 90, 'Por que verify', 'seção 4'),
  ...
  ('ci-q2-a07', 3,
   ...
   '["Marcar o passo com if: always(), para que ele execute mesmo depois da falha",
     ...
     "Trocar if-no-files-found para warn, porque error interrompe o passo de upload"]'::jsonb, 90, 'Preservar evidência', 'seção 6'),
  ...
  ('ci-q2-a07', 4,
   ...
   '["Nenhum, desde que o repositório do Foot Fanatics permaneça privado durante o semestre",
     ...
     "write-all é ignorado pelo GitHub Actions, então o passo falha por permissão ausente"]'::jsonb, 90, 'Segurança do workflow', 'seção 7'),
  ...
  ('ci-q2-a07', 6,
   ...
   '["O rerun confirma a intermitência; ele não corrige e não deveria liberar o merge",
     ...
     "O rerun invalida o resultado anterior, então o vermelho pode ser removido do histórico"]'::jsonb, 90, 'Flakiness e diagnóstico', 'seção 9'),
  ...
  ('ci-q2-a07', 7,
   ...
   '["Aceitar, desde que o registro do débito seja aberto e tratado no ciclo seguinte",
     ...
     "Recusar: se o pipeline está lento ou instável, corrija o pipeline, não a regra"]'::jsonb, 90, 'Gate como política', 'seção 8'),
  ...
  ('ci-q2-a07', 8,
   ...
   '["Reexecutar o job algumas vezes para verificar se o resultado se mantém vermelho",
     ...
     "Limpar o cache do Maven, porque dependência antiga costuma quebrar só na CI"]'::jsonb, 90, 'Passa local, falha na CI', 'seção 9')
  returning id, ordem
)
